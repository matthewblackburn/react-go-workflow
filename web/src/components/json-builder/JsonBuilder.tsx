                    ...
                    <>
                        ...
                        {valueMenuItems.map((mi) => (
                            <button
                                ...
                                className="flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-xs hover:bg-accent"
                            >
                                <span className="flex h-2.5 w-2.5 shrink-0 items-center justify-center">{mi.icon}</span>
...
function parseArrayItems(itemSchema: Record<string, any>): SchemaField {
    return {
        ...
        description: "",
        properties: itemSchema.type === "object" && itemSchema.properties ? schemaToTree(itemSchema) : undefined,
        items: itemSchema.type === "array" && itemSchema.items ? parseArrayItems(itemSchema.items) : undefined,
    };
}
...
function TreeConnector() {
    return (
        <span className="flex shrink-0 items-center" style={{ width: "18px", marginLeft: "-18px" }}>
...
function CollapsedValue({
    ...
    if (matchedItem) {
        return (
            <div className="ml-1 LLM provider is not configured or does not support the configured model min-w-[40px] flex-1">
                {matchedItem.render({
    ...
    return (
        <button
            type="button"
            onClick={onEdit}
            className="ml-1 h-5 truncate rounded px-1.5 text-[11px] text-muted-foreground leading-5 transition-colors hover:bg-muted/50"
        >
            {field.type === "boolean" ? (
                field.default ? (
                    "true"
...
function ValueInput({
    ...
    if (field.type === "boolean") {
        return (
            <button
                ...
                className={`h-5 rounded px-2 font-medium text-[10px] leading-5 ${
                    field.default
    ...
    if (field.type === "number") {
        return (
            <Input
                ...
                placeholder="value"
                className="!h-5 !text-[11px] !min-h-0 !py-0 !shadow-none !border-0 !leading-5 w-16 shrink-0 rounded bg-muted/30 px-1.5 focus-visible:bg-muted/50 focus-visible:ring-0"
            />
        );
    }
    ...
    if (matchedItem) {
        return (
            <div className="ml-1 min-w-[40px] flex-1">
                {matchedItem.render({
                    value: String(field.default ?? ""),
                    onChange: (v) => update({ default: v || undefined }),
                })}
    ...
    return (
        <Input
            ...
            placeholder="value"
            className="!h-5 !text-[11px] !min-h-0 !py-0 !shadow-none !border-0 !leading-5 min-w-[40px] flex-1 rounded bg-muted/30 px-1.5 focus-visible:bg-muted/50 focus-visible:ring-0"
        />
    );
}
...
function FieldRow({
    ...
    const isDuplicate =
        !isArrayItem && field.name.trim() !== "" && (siblingNames?.filter((n) => n === field.name).length ?? 0) > 1;
                ...
                <div className="group flex items-center gap-1 py-[3px]">
                    ...
                    {rules.editableKey ? (
                        <button
                            ...
                            className={`h-5 truncate rounded px-1.5 text-left font-mono text-[11px] leading-5 transition-colors hover:bg-muted/50 ${
                        ...
                        <span
                            className={`h-5 truncate px-1.5 font-mono text-[11px] text-foreground leading-5 ${isDuplicate ? "text-red-500" : ""}`}
                        >
                            {field.name}
                        </span>
                    )}
                    ...
                    {isContainer && (
                        <button
                            ...
                            className="ml-1 font-mono text-[11px] text-muted-foreground transition-colors hover:text-foreground"
                        >
                            {typeInfo.bracket}
                        </button>
                    )}
    ...
    return (
        <div className="relative">
            ...
            >
                ...
                {isArrayItem ? (
                    <span className="shrink-0 pl-1 text-[11px] text-muted-foreground italic">items</span>
                ) : !rules.editableKey ? (
                    <span className="shrink-0 px-1.5 font-mono text-[11px] text-foreground">{field.name}</span>
                ) : (
                    <Input
                        ref={nameRef}
                        value={field.name}
                        ...
                        placeholder="name"
                        className={`!h-5 !text-[11px] !min-h-0 !py-0 !shadow-none !border-0 !leading-5 w-24 shrink-0 rounded border-none bg-muted/30 px-1.5 font-mono shadow-none focus-visible:bg-muted/50 focus-visible:ring-0 ${
                            isDuplicate ? "bg-red-500/10 text-red-500" : ""
                        }`}
                    />
                )}
                ...
                {isArrayItem && !!rules.canChangeType && (
                    <button
                        ...
                        className="flex shrink-0 items-center gap-1 rounded px-1.5 py-0.5 text-[10px] transition-colors hover:bg-muted"
                    >
                ...
                {!isContainer && !isArrayItem && rules.showValue && (
                    <ValueInput field={field} valueRef={valueRef} update={update} valueMenuItems={valueMenuItems} />
                )}
...
function ObjectChildren({
    ...
    return (
        <div className="ml-7">
            <div ref={containerRef} className="relative" style={{ marginLeft: "-18px", paddingLeft: "18px" }}>
                <div className="absolute w-px bg-border/50" style={{ left: 0, top: 0, height: lineHeight ?? 0 }} />
            ...
            {rules.canAddFields && (
                <>
                    <button
                        ...
                        className="ml-1 flex items-center gap-1 py-1 text-[10px] text-muted-foreground transition-colors hover:text-foreground"
                    >
        ...
        <div>
            {fields.map((field, i) => (
                <FieldRow // biome-ignore lint/suspicious/noArrayIndexKey: fields can have empty names
                    ...
                    onRemove={
                        !rules.canRemoveFields ? undefined : () => updateAndEmit(fields.filter((_, j) => j !== i))
                    }
                />
            ))}
            ...
            {!rules.canAddFields ? (
                fields.length === 0 ? (
                    <p className="py-3 text-muted-foreground text-xs italic">No fields defined</p>
                ) : null
            ) : (
                <>
                    <div className="mt-1 flex items-center gap-3 border-border/30 border-t pt-2">
                        <button
                            ref={addRef}
                            type="button"
                            onClick={() => setAddPickerOpen(true)}
                        ...
                        {fields.length > 0 && (
                            <>
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-1 text-[10px] text-muted-foreground transition-colors hover:text-red-500"
                                >
