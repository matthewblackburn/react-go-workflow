...
func (_q *NotificationQuery) QueryWorkflow() *WorkflowQuery {
...
func (_q *NotificationQuery) QueryWorkflowExecution() *WorkflowExecutionQuery {
...
func (_q *NotificationQuery) WithWorkflow(opts ...func(*WorkflowQuery)) *NotificationQuery {
	query := (&WorkflowClient{config: _q.config}).Query()
...
func (_q *NotificationQuery) WithWorkflowExecution(opts ...func(*WorkflowExecutionQuery)) *NotificationQuery {
	query := (&WorkflowExecutionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(notification.FieldDateCreated).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Notification.Query().
//		Select(notification.FieldDateCreated).
//		Scan(ctx, &v)
...
func (_q *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	...
	if query := _q.withWorkflow; query != nil {
		if err := _q.loadWorkflow(ctx, query, nodes, nil,
			func(n *Notification, e *Workflow) { n.Edges.Workflow = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withWorkflowExecution; query != nil {
		if err := _q.loadWorkflowExecution(ctx, query, nodes, nil,
			func(n *Notification, e *WorkflowExecution) { n.Edges.WorkflowExecution = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NotificationQuery) loadWorkflow(ctx context.Context, query *WorkflowQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Workflow)) error {
...
func (_q *NotificationQuery) loadWorkflowExecution(ctx context.Context, query *WorkflowExecutionQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *WorkflowExecution)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Notification)
	for i := range nodes {
...
func (_q *NotificationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withWorkflow != nil {
			_spec.Node.AddColumnOnce(notification.FieldWorkflowID)
		}
		if _q.withWorkflowExecution != nil {
			_spec.Node.AddColumnOnce(notification.FieldWorkflowExecutionID)
		}
	}
