...
func (tx *Tx) init() {
	tx.CanvasNote = NewCanvasNoteClient(tx.config)
	...
	tx.StepExecution = NewStepExecutionClient(tx.config)
	tx.StepType = NewStepTypeClient(tx.config)
...
// applies a query, for example: CanvasNote.QueryXXX(), the query will be executed
