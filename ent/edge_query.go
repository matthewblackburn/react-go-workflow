...
type EdgeQuery struct {
	...
	withSourceStep *StepQuery
	withTargetStep *StepQuery
...
func (_q *EdgeQuery) QueryWorkflow() *WorkflowQuery {
...
// QuerySourceStep chains the current query on the "source_step" edge.
func (_q *EdgeQuery) QuerySourceStep() *StepQuery {
...
// QueryTargetStep chains the current query on the "target_step" edge.
func (_q *EdgeQuery) QueryTargetStep() *StepQuery {
	query := (&StepClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *EdgeQuery) Clone() *EdgeQuery {
	...
	return &EdgeQuery{
		...
		withSourceStep: _q.withSourceStep.Clone(),
		withTargetStep: _q.withTargetStep.Clone(),
...
func (_q *EdgeQuery) WithWorkflow(opts ...func(*WorkflowQuery)) *EdgeQuery {
	query := (&WorkflowClient{config: _q.config}).Query()
...
// WithSourceStep tells the query-builder to eager-load the nodes that are connected to
// the "source_step" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *EdgeQuery) WithSourceStep(opts ...func(*StepQuery)) *EdgeQuery {
	query := (&StepClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSourceStep = query
	return _q
}

// WithTargetStep tells the query-builder to eager-load the nodes that are connected to
// the "target_step" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *EdgeQuery) WithTargetStep(opts ...func(*StepQuery)) *EdgeQuery {
	query := (&StepClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTargetStep = query
	return _q
}
...
func (_q *EdgeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Edge, error) {
	var (
		...
		loadedTypes = [3]bool{
			_q.withWorkflow != nil,
			_q.withSourceStep != nil,
			_q.withTargetStep != nil,
		}
	)
	...
	if query := _q.withWorkflow; query != nil {
		if err := _q.loadWorkflow(ctx, query, nodes, nil,
			func(n *Edge, e *Workflow) { n.Edges.Workflow = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSourceStep; query != nil {
		if err := _q.loadSourceStep(ctx, query, nodes, nil,
			func(n *Edge, e *Step) { n.Edges.SourceStep = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTargetStep; query != nil {
		if err := _q.loadTargetStep(ctx, query, nodes, nil,
			func(n *Edge, e *Step) { n.Edges.TargetStep = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *EdgeQuery) loadWorkflow(ctx context.Context, query *WorkflowQuery, nodes []*Edge, init func(*Edge), assign func(*Edge, *Workflow)) error {
...
func (_q *EdgeQuery) loadSourceStep(ctx context.Context, query *StepQuery, nodes []*Edge, init func(*Edge), assign func(*Edge, *Step)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Edge)
	for i := range nodes {
		fk := nodes[i].SourceStepID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "source_step_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *EdgeQuery) loadTargetStep(ctx context.Context, query *StepQuery, nodes []*Edge, init func(*Edge), assign func(*Edge, *Step)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Edge)
	for i := range nodes {
		fk := nodes[i].TargetStepID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "target_step_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *EdgeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withWorkflow != nil {
			_spec.Node.AddColumnOnce(edge.FieldWorkflowID)
		}
		if _q.withSourceStep != nil {
			_spec.Node.AddColumnOnce(edge.FieldSourceStepID)
		}
		if _q.withTargetStep != nil {
			_spec.Node.AddColumnOnce(edge.FieldTargetStepID)
		}
	}
